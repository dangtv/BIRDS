...
Cache.prototype.sqlFilePath = function sqlFilePath() {
  return path.join(dbPath, '/cache/', this.cacheKey + '.sql');
};
...
Cache.prototype.writeSql = function writeSql(datalogResult) {
  const self = this;
  return new Promise(resolve => {
      fs.writeFile(self.sqlFilePath(), datalogResult.sql, function(err) {
        if (err) {
          console.log(err);
        }
        return resolve();
